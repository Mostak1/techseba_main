...
@section('title')
    <title>{{ __('translate.Job Categories') }}</title>
@endsection

@section('body-header')
    <h3 class="crancy-header__title m-0">{{ __('translate.Job Categories') }}</h3>
    <p class="crancy-header__text">{{ __('translate.Dashboard') }} >> {{ __('translate.Job Categories') }}</p>
@endsection
                                ...
                                <div class="crancy-customer-filter">
                                    <div class="crancy-customer-filter__single crancy-customer-filter__single--csearch d-flex items-center justify-between create_new_btn_box">
                                        <div class="crancy-header__form crancy-header__form--customer create_new_btn_inline_box">
                                            <h4 class="crancy-product-card__title">{{ __('translate.Job Categories') }}</h4>
                                            <a href="{{ route('admin.categories.create') }}" class="crancy-btn">
                                                <span>
                                        ...
                                        <tbody class="crancy-table__body">
                                            @foreach ($categories as $index => $cat)
                                                <tr class="odd">
                                                    ...
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        <a href="{{ route('admin.categories.edit', $cat->id) }}" class="crancy-btn"><i class="fas fa-edit"></i> {{ __('translate.Edit') }}</a>
                                                        <a onclick="itemDeleteConfirmation({{ $cat->id }})" href="javascript:;" data-bs-toggle="modal" data-bs-target="#deleteModal" class="crancy-btn delete_danger_btn"><i class="fas fa-trash"></i> {{ __('translate.Delete') }}</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
@push('js_section')
    <script>
        ...
        function itemDeleteConfirmation(id) {
            $("#item_delete_form").attr("action", '{{ url("admin/jobs-management/categories") }}/' + id);
        }
    </script>
@endpush
