...
@section('title')
    <title>{{ __('translate.Jobs List') }}</title>
@endsection

@section('body-header')
    <h3 class="crancy-header__title m-0">{{ __('translate.Jobs List') }}</h3>
    <p class="crancy-header__text">{{ __('translate.Dashboard') }} >> {{ __('translate.Jobs List') }}</p>
@endsection
                                ...
                                <div class="crancy-customer-filter">
                                    <div class="crancy-customer-filter__single crancy-customer-filter__single--csearch d-flex items-center justify-between create_new_btn_box">
                                        <div class="crancy-header__form crancy-header__form--customer create_new_btn_inline_box">
                                            <h4 class="crancy-product-card__title">{{ __('translate.Jobs List') }}</h4>
                                            <a href="{{ route('admin.jobs.create') }}" class="crancy-btn">
                                                <span>
                                    ...
                                    <table class="crancy-table__main crancy-table__main-v3 no-footer" id="dataTable">
                                        <thead class="crancy-table__head">
                                            <tr>
                                                ...
                                                <th class="crancy-table__column-2 crancy-table__h2">{{ __('translate.Category') }}</th>
                                        ...
                                        <tbody class="crancy-table__body">
                                            @foreach ($jobs as $index => $job)
                                                <tr class="odd">
                                                    ...
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        <h4 class="crancy-table__product-title">{{ $job->organization->name }}</h4>
                                                    </td>
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        <h4 class="crancy-table__product-title">{{ $job->category->name }}</h4>
                                                    </td>
                                                    <td class="crancy-table__column-2 crancy-table__data-2 font-weight-bold text-capitalize">
                                                    ...
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        <a href="{{ route('admin.jobs.edit', $job->id) }}" class="crancy-btn"><i class="fas fa-edit"></i> {{ __('translate.Edit') }}</a>
                                                        <a onclick="itemDeleteConfirmation({{ $job->id }})" href="javascript:;" data-bs-toggle="modal" data-bs-target="#deleteModal" class="crancy-btn delete_danger_btn"><i class="fas fa-trash"></i> {{ __('translate.Delete') }}</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
@push('js_section')
    <script>
        ...
        function itemDeleteConfirmation(id) {
            $("#item_delete_form").attr("action", '{{ url("admin/jobs-management/jobs") }}/' + id);
        }
    </script>
@endpush
