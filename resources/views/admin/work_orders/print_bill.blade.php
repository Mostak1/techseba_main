...
<title>TechSeba Invoice - {{ $bill->bill_number }}</title>
<style>
  :root {
    --navy:#0b2c72;
    --blue:#0e6fe8;
...
<section class="page">
  ...
  <div class="header">
    ...
    <div class="doc-title">
      ...
      <div class="meta">
        ...
        <strong>Date Generated:</strong> {{ $bill->created_at->format('M d, Y') }}<br>
  ...
  <div class="two-col">
    <div class="info-box">
      ...
      <div class="info-line"><strong>Name:</strong> {{ $bill->workOrder->user->name }}</div>
      <div class="info-line"><strong>Email:</strong> {{ $bill->workOrder->user->email }}</div>
      <div class="info-line"><strong>Phone:</strong> {{ $bill->workOrder->user->phone ?? 'N/A' }}</div>
      <div class="info-line"><strong>Address:</strong> {{ $bill->workOrder->user->address ?? 'N/A' }}</div>
    </div>
    <div class="info-box">
  ...
  <div class="section-title">1. ASSOCIATED WORK ORDER & DETAILS</div>
  <div class="project-title">
    Work Order: {{ $bill->workOrder->title }} ({{ $bill->workOrder->order_number }})
  </div>
  ...
  @if($bill->notes)
    <div style="font-size: 13px; line-height: 1.6; color: #334155; background: #faf5ff; border-left: 4px solid var(--purple); padding: 15px; border-radius: 6px; margin: 15px 0;">
      <strong>Invoice Description / Note:</strong><br>
      {!! nl2br(e($bill->notes)) !!}
    </div>
  @endif

  <div class="section-title">2. BILLING & PAYMENT STATUS</div>
  <table>
    <thead>
      <tr>
    ...
    <tbody>
      <tr>
        ...
        <td class="amount">{{ currency($bill->amount, 2) }}</td>
      </tr>
      <tr style="background-color: #f8fafc; font-weight: bold;">
        <td colspan="2">Total Invoice Amount Due</td>
        <td class="amount">{{ currency($bill->amount, 2) }}</td>
      </tr>
      <tr style="border-top: 2px solid var(--navy);">
        <td colspan="2"><strong>Status</strong></td>
        <td class="amount" style="text-transform: uppercase;">
          @if($bill->status == 'paid')
            <span style="color: #16a34a;">PAID / CONFIRMED</span>
          @else
  ...
  @if($bill->status == 'paid')
  <div style="border: 1px dashed #16a34a; background: #f0fdf4; border-radius: 6px; padding: 15px; margin: 15px 0; font-size: 13px;">
    <h4 style="margin: 0 0 5px 0; color: #166534; font-size: 15px;">Payment Details:</h4>
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
      <div><strong>Payment Method:</strong> {{ $bill->payment_method }}</div>
      <div><strong>Transaction ID:</strong> {{ $bill->transaction_id ?? 'N/A' }}</div>
      <div><strong>Payment Date:</strong> {{ $bill->payment_date ? $bill->payment_date->format('M d, Y h:i A') : 'N/A' }}</div>
    </div>
  </div>
  @endif

  <div class="note">
    Please note: Make payments to the official channels. Quote the Invoice/Bill Number during transaction reports. For billing disputes, contact +8801898828248.
  </div>
