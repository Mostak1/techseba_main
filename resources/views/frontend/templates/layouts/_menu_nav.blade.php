<div class="menu-block-wrapper">
    ...
    <nav class="menu-block" id="append-menu-header">
        ...
        <ul class="site-menu-main {{ $menuTheme ?? '' }}">
            ...
            @foreach ($enabled_pages ?? [] as $enabledPage)
                @unless($enabledPage->is_managed)
                    <li class="nav-item">
                        <a href="{{ $enabledPage->menu_url }}" class="nav-link-item">
                            <span class="menu-item-text">{{ $enabledPage->name }}</span>
                        </a>
                    </li>
                @endunless
            @endforeach
            ...
            @php
                $customPageCount = ($custom_pages ?? collect())->count();
                $showUtilityDropdown = page_enabled('faq') || page_enabled('testimonials');
                $showPagesDropdown = page_enabled('about-us') || page_enabled('pricing-plan') || page_enabled('teams') || $showUtilityDropdown || $customPageCount > 0;
            @endphp
