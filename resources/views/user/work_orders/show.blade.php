...
@section('dashboard-content')
    <div style="background: white; padding: 25px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 30px;">
        ...
        <div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-top: 30px;">
            <h4 style="font-size: 16px; font-weight: 600; margin-bottom: 15px; color: #1e293b; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">Recurring & Extra Bills</h4>
            <div class="table-responsive">
                <table class="table" style="width: 100%; border-collapse: collapse; font-size: 14px;">
                    <thead>
                    <tr style="border-bottom: 2px solid #edf2f7; text-align: left; background-color: #f8fafc;">
                    ...
                    @forelse($workOrder->bills as $bill)
                        <tr style="border-bottom: 1px solid #edf2f7;">
                            <td style="padding: 10px;"><strong>{{ $bill->bill_number }}</strong></td>
                            <td style="padding: 10px;">{{ $bill->title }}</td>
                            <td style="padding: 10px; text-transform: capitalize;">{{ str_replace('_', ' ', $bill->bill_type) }}</td>
                            <td style="padding: 10px; font-weight: 600;">{{ currency($bill->amount, 2) }}</td>
                            <td style="padding: 10px;">{{ $bill->due_date->format('M d, Y') }}</td>
                            <td style="padding: 10px;">
                                @if($bill->status == 'paid')
                                    <span class="badge" style="background-color: #d1fae5; color: #065f46; padding: 4px 8px; border-radius: 4px; font-size: 11px;">Paid</span>
                                @else
                                    <span class="badge" style="background-color: #fee2e2; color: #991b1b; padding: 4px 8px; border-radius: 4px; font-size: 11px;">Unpaid</span>
                                @endif
                            </td>
                            <td style="padding: 10px;">
                                <a href="{{ route('user.work_orders.bills.print', $bill->id) }}" target="_blank" class="btn btn-sm btn-outline-primary" style="border: 1px solid #4f46e5; color: #4f46e5; background: transparent; padding: 4px 8px; border-radius: 4px; text-decoration: none; font-size: 12px; display: inline-flex; align-items: center; gap: 4px;">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center" style="padding: 20px; color: #64748b;">No bills generated yet.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
